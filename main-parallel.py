...
async def main():
    ...
    short_test_file = './short-possibilities.csv'
    
    # Configuration
    MAX_CONCURRENT = 200  # adjust down if we get rate limited
    BATCH_SIZE = 200      # process this many records before saving progress
    
    # Determine which file to process
    files_to_process = [nursing_file, physician_file]
    # files_to_process = [short_test_file]  

    timestamp = datetime.now().strftime("%d-%m-%y-%H-%M")
    for file_path in files_to_process:
        # base_name = os.path.splitext(os.path.basename(file_path))[0]
        output_file = f"possibilities-{timestamp}.csv"
